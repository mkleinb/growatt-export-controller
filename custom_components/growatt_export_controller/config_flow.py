...
async def _validate_login(hass, data: dict[str, Any]) -> None:
    ...
    client = GrowattApiClient(
        ...
        GrowattClientConfig(
            ...
            login_base_url=str(data.get(CONF_COMMAND_BASE_URL, DEFAULT_COMMAND_BASE_URL)).rstrip("/"),
...
class GrowattExportControllerOptionsFlow(OptionsFlowWithReload):
    ...
    async def async_step_init(self, user_input: dict[str, Any] | None = None) -> ConfigFlowResult:
        ...
        if user_input is not None:
            ...
            strategy = str(user_input.get(CONF_PRICE_STRATEGY, DEFAULT_PRICE_STRATEGY))
            ...
            if strategy == "manual" and recovery < activation:
                errors[CONF_PRICE_RECOVERY_THRESHOLD] = "recovery_below_activation"

            if strategy == "economic_auto":
                # Automatic economic mode always uses the proven Growatt
                # zero-export policy while preserving solar self-consumption.
                user_input[CONF_PRICE_TRIGGER_METER_ENABLED] = True
                user_input[CONF_PRICE_TRIGGER_EXPORT_PERCENTAGE] = 0
                user_input[CONF_PRICE_NORMAL_METER_ENABLED] = False
                user_input[CONF_PRICE_NORMAL_EXPORT_PERCENTAGE] = 100
        ...
        schema = vol.Schema(
            {
                ...
                vol.Optional(
                    CONF_PRICE_STRATEGY,
                    default=data.get(CONF_PRICE_STRATEGY, DEFAULT_PRICE_STRATEGY),
                ): selector.SelectSelector(
                    selector.SelectSelectorConfig(
                        options=[
                            selector.SelectOptionDict(
                                value="economic_auto",
                                label="Automatic economic break-even",
                            ),
                            selector.SelectOptionDict(
                                value="manual",
